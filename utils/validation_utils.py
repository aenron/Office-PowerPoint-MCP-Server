    ...
    try:
        ...
        if hasattr(shape, 'width') and hasattr(shape, 'height'):
            ...
            if estimated_width > shape_width_pt:
                ...
                suggested_size = int(
                    (shape_width_pt / len(text_content)) * 0.8)
        ...
        if max_line_length > 100:  # Arbitrary threshold
            result['warnings'].append(
                "Very long lines detected. Consider adding line breaks.")
            result['needs_optimization'] = True
    ...
    try:
        ...
        for shape_index, shape in shapes_with_text:
            ...
            if len(shape.text_frame.text) > 500:  # Very long text
                result['warnings'].append(
                    f"{shape_name}: Contains very long text (>500 chars)")

            # Check for empty paragraphs
            empty_paragraphs = sum(
                1 for p in shape.text_frame.paragraphs if not p.text.strip())
            if empty_paragraphs > 2:
                result['warnings'].append(
                    f"{shape_name}: Contains {empty_paragraphs} empty paragraphs")

        # Check slide-level issues
        if len(slide.shapes) > 20:
            result['warnings'].append(
                "Slide contains many shapes (>20), may affect performance")
    ...
    try:
        ...
        if result['overlapping_shapes']:
            ...
            result['issues'].append(
                f"Found {len(result['overlapping_shapes'])} overlapping shapes")
            result['suggestions'].append(
                "Consider repositioning overlapping shapes")
        ...
        if shapes_outside:
            result['layout_valid'] = False
            result['issues'].append(
                f"Found {len(shapes_outside)} shapes outside slide boundaries")
            result['suggestions'].append(
        ...
        if len(shapes) > 1:
            ...
            if min_spacing < 0.1 * 914400:  # Less than 0.1 inch spacing
                result['suggestions'].append(
                    "Consider increasing spacing between shapes")
