        ...
        for effect_name in effects:
            ...
            try:
                if effect_type == 'shadow':
                    self._apply_text_shadow(
                        text_frame, effect_config, color_scheme)
                elif effect_type == 'glow':
                    self._apply_text_glow(
                        text_frame, effect_config, color_scheme)
            ...
            if 'border' in effect_config:
                ...
                if 'color_role' in border_config:
                    color = self._get_color_from_scheme(
                        color_scheme, border_config['color_role'])
                    image_shape.line.color.rgb = RGBColor(*color)
                elif 'color' in border_config:
                    image_shape.line.color.rgb = RGBColor(
                        *border_config['color'])
...
class EnhancedTemplateManager:
    ...
    def get_dynamic_font_size(self, element: Dict, content: str = None) -> int:
        ...
        base_sizes = sizing_rules.get(
            'text_measurement', {}).get('base_font_sizes', {})
        ...
        try:
            ...
            if background_config:
                apply_slide_background(
                    slide, background_config, self.templates_data, color_scheme)
            ...
            for element in template.get('elements', []):
                ...
                try:
                    ...
                    if element_type == 'text':
                        ...
                        created_element = create_shape_element(
                            slide, element, self.templates_data, color_scheme)
                    elif element_type == 'image':
                        image_path = image_paths.get(
                            element_role) if image_paths else None
                        created_element = create_image_element(
                            slide, element, image_path)
                    elif element_type == 'table':
                        created_element = create_table_element(
                            slide, element, self.templates_data, color_scheme)
                    elif element_type == 'chart':
                        created_element = create_chart_element(
                            slide, element, self.templates_data, color_scheme)
    ...
    def create_enhanced_text_element(self, slide, element: Dict, templates_data: Dict,
        ...
        if styling.get('auto_wrap', False):
            ...
            content = self.text_calculator.wrap_text_intelligently(
                content, container_width, font_size)
        ...
        self.apply_enhanced_text_styling(
            textbox.text_frame, element, templates_data, color_scheme, font_size)
    ...
    def apply_enhanced_text_styling(self, text_frame, element: Dict, templates_data: Dict,
        ...
        typography_style = templates_data.get(
            'typography_styles', {}).get('modern_sans', {})
        font_type = styling.get('font_type', 'body')
        font_config = typography_style.get(
            font_type, {'name': 'Segoe UI', 'weight': 'normal'})

        # Color handling
        color = None
        if 'color_role' in styling:
            color = get_color_from_scheme(
                templates_data, color_scheme, styling['color_role'])
        ...
        for paragraph in text_frame.paragraphs:
            ...
            for run in paragraph.runs:
                ...
                font.italic = styling.get(
                    'italic', font_config.get('style') == 'italic')
        ...
        if text_effects:
            self.effects_manager.apply_text_effects(
                text_frame, text_effects, color_scheme)
...
def apply_text_styling(text_frame, styling: Dict, templates_data: Dict, color_scheme: str) -> None:
    ...
    font_settings = get_font_settings(
        templates_data, font_type, font_size_category)
...
def create_text_element(slide, element: Dict, templates_data: Dict, color_scheme: str) -> Any:
    ...
    # Apply styling
    styling = element.get('styling', {})
    apply_text_styling(textbox.text_frame, styling,
                       templates_data, color_scheme)
...
def create_shape_element(slide, element: Dict, templates_data: Dict, color_scheme: str) -> Any:
    ...
    try:
        ...
        shape = add_shape_direct(
            slide, shape_type, pos['left'], pos['top'], pos['width'], pos['height'])
        ...
        if 'fill_color_role' in styling:
            fill_color = get_color_from_scheme(
                templates_data, color_scheme, styling['fill_color_role'])
            shape.fill.solid()
        ...
        if 'line_color_role' in styling:
            line_color = get_color_from_scheme(
                templates_data, color_scheme, styling['line_color_role'])
        ...
        for c in range(cols):
            ...
            if r == 0 and header_row:
                ...
                if 'header_bg_color_role' in styling:
                    bg_color = get_color_from_scheme(
                        templates_data, color_scheme, styling['header_bg_color_role'])
                    cell.fill.solid()
                    ...
                    for paragraph in cell.text_frame.paragraphs:
                        for run in paragraph.runs:
                            run.font.color.rgb = RGBColor(
                                *styling['header_text_color'])
                            run.font.bold = True
            else:
                # Body styling
                if 'body_bg_color_role' in styling:
                    bg_color = get_color_from_scheme(
                        templates_data, color_scheme, styling['body_bg_color_role'])
                    cell.fill.solid()
                    cell.fill.fore_color.rgb = RGBColor(*bg_color)
...
def apply_slide_template_basic(slide, template_id: str, color_scheme: str = 'modern_blue',
    ...
    try:
        ...
        if background_config:
            apply_slide_background(
                slide, background_config, templates_data, color_scheme)
        ...
        for element in template.get('elements', []):
            ...
            try:
                ...
                if element_type == 'text':
                    created_element = create_text_element(
                        slide, element, templates_data, color_scheme)
                elif element_type == 'image':
                    image_path = image_paths.get(
                        element_role) if image_paths else None
                    created_element = create_image_element(
...
def get_template_base_layout(presentation: Presentation):
    """Get the best base layout for template-driven slide creation."""
    blank_layout_names = {'blank', '空白'}

    for layout in presentation.slide_layouts:
        if layout.name.strip().lower() in blank_layout_names:
            return layout
    ...
    if len(presentation.slide_layouts) > 0:
        return presentation.slide_layouts[len(presentation.slide_layouts) - 1]
...
def create_presentation_from_template_sequence(presentation: Presentation, template_sequence: List[Dict],
    ...
    for i, slide_config in enumerate(template_sequence):
        try:
            ...
            # Add new slide using a blank-compatible base layout
            layout = get_template_base_layout(presentation)
