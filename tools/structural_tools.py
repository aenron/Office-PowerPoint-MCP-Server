...
def register_structural_tools(app: FastMCP, presentations: Dict, get_current_presentation_id, validate_parameters, is_positive, is_non_negative, is_in_range, is_valid_rgb, add_shape_direct):
    ...
    def add_table(
        ...
        header_bg_color: List[int] = [],
        body_bg_color: List[int] = [],
        ...
        if header_bg_color:
            validations["header_bg_color"] = (header_bg_color, [(
                is_valid_rgb, "must be a valid RGB list [R, G, B] with values 0-255")])
        if body_bg_color:
            validations["body_bg_color"] = (body_bg_color, [(
                is_valid_rgb, "must be a valid RGB list [R, G, B] with values 0-255")])
        if border_color:
            validations["border_color"] = (border_color, [(
                is_valid_rgb, "must be a valid RGB list [R, G, B] with values 0-255")])
                ...
                for c in range(cols):
                    ...
                    if r == 0 and header_row:
                        if header_bg_color:
                            ...
                            ppt_utils.format_table_cell(
                                cell, font_size=header_font_size, bold=True)
                    else:
    ...
    def add_shape(
        ...
        try:
            ...
            if fill_color or line_color or line_width > 0:
                ppt_utils.format_shape(
                    ...
                    line_width=line_width if line_width > 0 else None
                )

            # Add text to shape if provided
            if text and hasattr(shape, 'text_frame'):
                shape.text_frame.text = text
                if font_size > 0 or font_color:
